...
### 修复
- **列表视图被超长系统名撑出横向滚动条**：一台机器的系统名过长（如 `Microsoft Windows Server 2022 Datacenter 21H2`）会把整张表格撑宽、底部出现横向滚动条。现从两处解决——系统名在列表与卡片中精简显示，去掉不影响判读的冗余部分（`Microsoft` 前缀、系统内部版本号与 Server 版本类型），上例显示为 `Windows Server 2022`；系统列同时限定宽度、超出以 `…` 截断兜底。完整名称悬停可见，详情页照旧显示完整信息，其余列宽不受影响。Linux 发行版的版本号是关键信息，一律原样保留。
